...
def check_password():
    ...
    if password_input or st.button("Einloggen", type="primary"):
        # Admin Login
        if password_input == st.secrets["APP_PASSWORD"]:
            ...
            st.rerun()
        # User Login
        elif password_input == st.secrets["USER_PASSWORD"]:
            st.session_state.password_correct = True
            st.session_state.is_admin = False
            st.rerun()
        else:
...
if check_password():
    st.set_page_config(page_title="Patent Manager Pro", layout="wide", page_icon="📊")
