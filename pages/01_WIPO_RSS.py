        ...
        if not df_disp.empty and "source_name" in df_disp.columns:
            for q, group in df_disp.groupby("source_name"):
                with st.expander(f"📂 {q} ({len(group)})", expanded=(st.session_state.get("active_folder") == q)):
                    if st.session_state.is_admin:
                        # --- NEUE SICHERHEITSABFRAGE START ---
                        with st.popover(f"🗑️ Ordner leeren"):
                            st.write(f"Sollen wirklich alle {len(group)} Einträge gelöscht werden?")
                            if st.button(f"Ja, '{q}' leeren", key=f"bulk_{q}", type="primary"):
